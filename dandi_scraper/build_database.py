...
def build_dataset_traces(folder, ext="nwb", parallel=False):
    ...
    if parallel == True:
        #Run in parallel
        parallel = joblib.cpu_count()
    results = joblib.Parallel(n_jobs=1, backend='multiprocessing')(joblib.delayed(plot_data)(specimen_id, files) for specimen_id in file_idx)

def plot_data(specimen_id, file_list=None, amp_interval=20, max_above_rheo=100, debug=True):
    result = {}
    result["specimen_id"] = file_list[specimen_id]
    _, _, _, _, data_set = loadNWB(file_list[specimen_id], return_obj=True)
    if data_set is None or len(data_set.dataY)<1:
